...
- **PTY idle notifications** — Background workspaces fire an OS desktop notification (via `notify-rust`) when their dispatched agent stops producing output for the configured threshold (default 3s); only triggers for custom providers (Claude/Gemini/etc.), not Shell tabs; sidebar shows a yellow `●` badge next to the workspace name until the user visits that workspace. Per-provider thresholds and on/off toggles live in `providers.toml` (`idle_threshold_secs`, `idle_notify`); the watcher itself lives in `piki-core::idle_watcher` and is shared by TUI and Desktop
- **Shell integration (Linux/macOS)** — Shell tabs (zsh, bash) auto-source a tiny init script that emits OSC 133 (prompt/command markers + exit code) and OSC 7 (cwd reporting). Piki's per-tab OSC parser captures those markers from the PTY stream and surfaces them: cwd of the active shell tab in the desktop status bar, ✓/✗ exit-code badge on the shell tab after each command, and a workspace `●` badge when a command finishes in a background tab. The init scripts live in `crates/core/src/shell_integration/scripts/` and are materialized to `<data_dir>/shell-integration/` on first use; bridge files chain to your real `~/.zshrc` / `~/.bashrc` so user dotfiles are preserved. Disabled gracefully for unsupported shells (`sh`, `dash`, `fish`)
...
- **Recursive split-view panes** — Split any tab horizontally or vertically into nested panes; click the `▾` button on a tab (or right-click) for **Split Right** / **Split Down** / **Move to Pane N** / **Close**; keyboard shortcuts `Ctrl+\` (split right), `Ctrl+Shift+\` (split down), `Ctrl+Shift+Q` (close active pane); each pane has its own mini tab bar with independent active tab and `+` button; drag the divider between panes to resize (clamped 10–90%); the active pane is highlighted with a subtle tint on its tab bar; the pane layout persists per workspace across restarts (keyed by workspace path)
