...
- **AI Chat** — Global chat panel powered by local LLMs via [Ollama](https://ollama.ai/); `Ctrl+Y` in TUI opens a centered floating overlay, `Ctrl+Shift+L` in desktop toggles a right-side panel; not tied to any workspace — conversation persists across workspace switches; select from installed Ollama models via Tab (TUI) or dropdown (Desktop); streaming responses with token-by-token rendering; clear with `Ctrl+L`; config (model, base URL, system prompt) persisted in settings
...
- **AI Chat panel** — Right-side chat panel (`Ctrl+Shift+L`) for chatting with local LLMs via Ollama; model selector dropdown populated from installed models, streaming responses, resizable panel, conversation persists when hidden; config saved to settings
