...
- **`rusqlite` pinned at 0.39** — not the latest 0.40.x. `libsqlite3-sys` 0.38.1 (pulled in by rusqlite ≥0.40.0) calls the still-unstable `cfg_select!` macro in its build script and fails to compile on stable rustc. Bump `crates/core/Cargo.toml` once `libsqlite3-sys` ships a working patch release. Separately, rusqlite ≥0.36 dropped `ToSql`/`FromSql` for `u64`, so `pr_number` is cast to/from `i64` at the SQLite boundary in `crates/core/src/storage/sqlite.rs`.
