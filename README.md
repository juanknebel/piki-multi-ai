...
- **Editor-style shell** — menu bar, activity bar, sidebar views (Explorer, Files, Source Control) with the Agents panel always docked below them, a status bar with branch / changes / cwd / agent status / sessions and a drop-down terminal (``Ctrl+Shift+` ``) rooted at `~` that is independent of any workspace, full keyboard accessibility with a visible focus ring everywhere
