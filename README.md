...
- **Dense workspace list** — One line per workspace: type icon (⎇ worktree, ▣ project, ○ simple) + name on the left, and right-aligned metadata only when it says something — worst agent status across the workspace's tabs (⚠ needs permission > ▷ running > ⏳ waiting > ✓ done), changed-file count (`3∆`), and ahead/behind (`↑1 ↓2`); alternating row backgrounds and uppercase group headers keep the hierarchy readable; press `Ctrl+G i` for full details (branch, paths, type, group, description, prompt) in a copyable overlay
