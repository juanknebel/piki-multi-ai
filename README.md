...
- **Editor-style shell** — menu bar, activity bar, sidebar views (Explorer, Files, Source Control) with the Agents panel always docked below them, a status bar with branch / changes / cwd / agent status / sessions and a one-click terminal that opens at `~` (home) regardless of the active worktree, full keyboard accessibility with a visible focus ring everywhere
