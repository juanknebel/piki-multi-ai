...
- **OS notifications** — Unified desktop-notification surface for tab-completion events, fired regardless of which workspace is currently focused. (1) Custom-provider tabs (Claude/Gemini/etc.) trigger when their `IdleWatcher` reports the PTY has been silent past the configured threshold (default 3s); the watcher also enforces a minimum re-arm byte delta (`DEFAULT_IDLE_REARM_BYTES = 256`) so cursor blinks, status redraws, and spinner frames at the agent's prompt don't cause repeated re-fires — the next notification only arrives after the agent produces a meaningful burst of new output. (2) Shell tabs trigger when an OSC 133 `command-end` marker arrives, with the workspace name and exit code in the body (`Command finished — <ws> — exit 0` / `Command failed (exit N) — <ws> — exit N`). The sidebar still marks a `●` badge per workspace for visual breadcrumb. Both helpers live in `piki-core::notifications` (`notify_agent_idle`, `notify_command_end`) and are shared by TUI and Desktop via a single `notify-rust` dependency in `piki-core`.
