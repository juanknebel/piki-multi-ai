...
- **Workspace dashboard** — Press `Ctrl+G b` (`Ctrl+G D` alias) for a bird's-eye overview of workspaces with open tabs (empty workspaces are hidden), grouped by worktree family with the parent first and its worktree children below; shows status (idle/busy/done), changed files, and ahead/behind; also lists **external `claude` agents** discovered via `/proc` scan (grouped by workspace via cwd, with subagents as `ppid` children) at the bottom; `j`/`k` to navigate, `Enter` to switch, `Esc` to close
...
- **Agents pane** — The bottom-left pane lists every running AI agent across ALL workspaces with its live status from the structured OSC 777 channel (running / needs permission / waiting / done) plus idle badges; `j`/`k` to select, `Enter` or click to jump straight to that workspace and tab; external `claude` agents (outside piki) are surfaced in the Dashboard (`Ctrl+G b`)
...
- **Agents panel** — Always-visible panel docked at the bottom of the sidebar, below whichever view is active — Explorer, Files, or Source Control (same layout as the TUI's bottom-left pane; defaults to at most 40% of the sidebar, resizable by dragging the divider above it, height persisted), listing the live AI agent tabs of **all** workspaces, not just the active one — the desktop counterpart of the TUI's bottom-left Agents pane. Also lists **external `claude` agents** (outside piki, via `/proc`) grouped by workspace with pid/cwd/subagents and a ⎘ button to open a terminal at that cwd. Each row shows workspace · provider, the structured cli-agent status when the channel reports (running / needs permission / waiting for input / needs you / done, with the last permission or response summary), or a plain alive/exited indicator otherwise; click a row to jump straight to that workspace and tab (focusing the pane that holds it). Header buttons dispatch a new agent (＋) or open the agent profile manager (⚙). The workspace list carries the same signal in miniature: each row (and each *collapsed* worktree family) gets a rollup glyph for its worst actionable agent state (⚠ needs permission / ● unseen news), ranked by the same severity rule the TUI uses (shared in `piki-core::cli_agent::status_severity`); looking at a tab acknowledges its "needs you" marker in both UIs
