...
- **PTY idle notifications** — Background workspaces fire an OS desktop notification (via `notify-rust`) when their dispatched agent stops producing output for 3+ seconds; only triggers for custom providers (Claude/Gemini/etc.), not Shell tabs; sidebar shows a yellow `●` badge next to the workspace name until the user visits that workspace
