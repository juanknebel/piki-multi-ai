    ...
    let keys = if state.editing_comment.is_some() {
        ...
        match state.focus {
            ReviewFocus::FileList => {
                "[j/k] navigate  [Enter] view diff  [l] diff pane  [s] submit  [r] refresh  [q] back  [[/]] resize".to_string()
            }
            ReviewFocus::DiffView => {
                format!(
                    "[j/k] cursor  [{}/{}] page  [g/G] top/bottom  [c] comment  [R] reply  [d] del comment  [h] files  [n/p] next/prev  [s] submit  [q] back",
                    config.format_binding("ctrl-d"),
