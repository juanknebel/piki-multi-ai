...
- **Binding-choice rule — no Shift where avoidable.** Default app chords MUST prefer an **unshifted lowercase** key. Do NOT bind a new action to an uppercase letter (`prefix-N`) or any Shift-requiring chord — every letter action is deliberately lowercase (`s r b m v` etc.). If every mnemonic lowercase key is taken, reach for `prefix-ctrl-<key>` before a Shift chord (Ctrl is acceptable; Shift is what we avoid). A Shift-requiring symbol is allowed ONLY for an entrenched convention (`:` palette, `?` help) or when it also carries an unshifted alternate in the array (e.g. `sidebar_grow = ["prefix->", "prefix-."]`). `}`/`{` (workspace next/prev) are the one accepted exception, kept by explicit user choice. `rename_tab = prefix-R` is the second intentional Shift exception (mnemonic `r` already taken by `clone_workspace` `prefix-r`); it is listed as `R` in the `docs/technical.md` prefix table and covered by `docs_parity`. Actions added after it followed the rule and took `prefix-ctrl-<key>` rather than a Shift chord: `jump_attention` `C-a` (`a` = about), `restart_tab` `C-r` (`r` = clone_workspace), `move_tab` `C-w` (`w` = switcher), `clear_terminal` `C-k` (`k` = focus_up, and it mirrors the desktop's Ctrl+Shift+K), `add_chat_context` `C-y` (`y` = chat panel). **Avoid `prefix-ctrl-i` and `prefix-ctrl-m`**: without the Kitty keyboard protocol they arrive as Tab and Enter, so the chord is indistinguishable from those keys.
- **`action_catalog.rs` is the single source of truth for every user-facing key.** The command palette, the which-key overlay, the `prefix-?` help browser and the `docs/technical.md` prefix table all derive from it — do not hand-add to any of them. Two axes describe an entry:
...
- The bottom-left pane is **Agents** (`ActivePane::Agents`): `App::agent_rows()` lists (workspace, tab) pairs with Custom providers, plus any tab whose cli-agent channel has reported — shell tabs get the FIFO + hook env too (`spawn_tab` Shell arm) and the shell bridge wraps a manual `claude` with `--settings $PIKI_CLAUDE_HOOK_SETTINGS`, so it shows up as `Claude (Shell)` after its first hook event (and drops off when the CLI exits: the shell's OSC 133 command-end clears `ShellTabState.cli_agent` — a Custom-provider tab runs the agent directly, never sees command-end, and always lists). `render_agents_pane` (ui/sidebar.rs) derives status live via `cli_agent_snapshot()` (rich status for agents with a hook bridge — Claude Code, Antigravity — or, passively, Codex via `piki_core::agent_state_detect` reading its OSC window-title/screen text) falling back to `agent_tab_indicator()` (bare PTY liveness: alive/exited) for everything else — if an agent shows `alive` instead of `running`/`idle`, its binary has neither a `piki_core::cli_agent::bridge_for_command()` entry nor an `agent_state_detect::manifest_for_command()` entry; `handle_agents_interaction` + `jump_to_agent` (input/interaction.rs) handle j/k/Enter; the `agents` config context holds its local keys. The highlighted row tracks the tab the user is standing on: `App::sync_agent_selection()` runs once per event-loop iteration and re-selects only when the active tab changed (so j/k browsing isn't yanked back, and a non-agent tab leaves the highlight put) — don't sync `selected_agent_row` at the individual tab/workspace switch sites instead, they're too many to keep in step. Tab label shown is `Tab::display_label()` (`custom_title` > `markdown_label` > `provider.label()`, 40-char cap, empty clears) — a renamed agent tab's custom title replaces the provider label in both the subtab bar (`ui/subtabs.rs`) and the Agents pane; rename via `prefix R` (`AppMode::RenameTab`/`DialogState::RenameTab` + `ui/dialogs/rename.rs` + `input/app_actions::open_rename_tab`/`handle_rename_tab_input`).
...
- `term_guard.rs` — Async-signal-safe terminal restoration on SIGTERM/SIGHUP/SIGINT/SIGQUIT (snapshots the cooked termios before `ratatui::init`, handlers write the reset sequences + `tcsetattr` and re-raise). The normal exit path and the panic hook still do the regular restore; this only covers `kill <pid>` after a hang
