...
- **Adding an app action**: add the default to `default_app()` (following the binding-choice rule above), the name to `APP_ACTIONS` in `input/mod.rs`, an arm in `dispatch_app_action()` calling a helper in `input/app_actions.rs`, an entry in `action_catalog.rs` (this drives BOTH the command palette and the which-key overlay — do not hand-add to `command_palette.rs`, it derives from the catalog), and a help line in `ui/dialogs/system.rs`. Mirror the default in the commented `config.example.toml` (repo root), and add a row to the README's prefix table between the `<!-- BEGIN:prefix-keys -->` markers — the `readme_parity` tests in `action_catalog.rs` fail the build if you don't. `prefix 1..9` (tab jump) is hardcoded in `handle_prefix_key`, not configurable.
