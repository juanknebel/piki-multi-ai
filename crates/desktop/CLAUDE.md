...
- `src/commands/session.rs` — persistent-session daemon queries backing the Sessions dialog (`list_sessions`/`kill_session`/`remove_session`; jump uses `local_workspace_idx`/`local_tab_idx`). Frontend: `dialogs/sessions-dialog.ts`, opened by `Alt+Shift+S` / View menu / palette. Adopting an orphan as a new tab is TUI-only for now.
