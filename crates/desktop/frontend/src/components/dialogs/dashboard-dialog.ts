...
export function showDashboard() {
    ...
    lastRepo = repo;
    {
      ...
      cardsHtml += `
        <div class="dash-card${isActive ? " dash-active" : ""}" data-idx="${idx}">
          ...
          ${branch || syncInfo ? `<div class="dash-card-branch" title="${branch ? esc(branch) : ""}">${branch ? icon("branch") + " " + esc(branchLabel(branch)) : ""}${syncInfo ? (branch ? " " : "") + syncInfo : ""}</div>` : ""}
  ...
  dialog.innerHTML = `
    <div class="ui-header">
      <span class="ui-header-title">Dashboard — ${totalShown} workspace${totalShown !== 1 ? "s" : ""}</span>
      <button data-variant="ghost" data-icon class="dialog-close ui-btn" title="Close" aria-label="Close">×</button>
    </div>
    <div class="dash-grid">${cardsHtml || '<div class="ui-empty">No open workspaces</div>'}</div>
  `;
