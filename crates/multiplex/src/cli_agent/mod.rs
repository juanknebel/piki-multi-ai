//! Structured CLI-agent lifecycle channel: types, JSON parsing, and the
//! per-tab FIFO transport ([`sock`]).
//!
//! This module carries one opinionated, built-in event vocabulary (matching
//! Claude Code's hook payloads) rather than a fully generic plugin system —
//! a real generic sidecar-event abstraction would need to reach into every
//! frontend's rendering code (including the desktop's TypeScript layer) to
//! stay behavior-preserving, which is out of scope for this extraction. A
//! consumer that doesn't wire up [`sock::spawn_reader`] (via
//! `cli_agent_sock: None` at spawn time) never sees any of these types, so
//! they're inert baggage rather than a hard dependency for a plain terminal
//! multiplexer use case.
//!
//! Piki's own hook-script installation (`install`/`install_antigravity`,
//! `AgentBridge`) stays in `piki-core` — only the wire types, the JSON
//! parser, and the FIFO reader live here, next to the PTY/session code that
//! consumes them.
