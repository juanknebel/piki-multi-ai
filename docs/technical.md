...
**Chat settings** (gear in the panel header): server (Ollama / llama.cpp / OpenRouter), base URL, system prompt and — for OpenRouter only — a *Web search* checkbox (the OpenRouter web plugin). Each backend remembers its own model, URL, prompt and web-search flag in `chat-providers.toml` (the file the TUI's chat settings use too): switching the server in the dropdown restores what you last used there, and *Save* stores the active config in the settings document plus that backend's entry in the file. The OpenRouter key is never entered here — only `[chat] openrouter_api_key` in `config.toml` or `OPENROUTER_API_KEY`.
...
**Where the files live** (`piki_core::agent_scan`, shared by both frontends): the *profiles* are keyed by `source_repo` — they belong to the repo and its whole worktree family — but the *files* are read from and written to **the checkout you are standing in**, not the parent repo. A worktree is its own working tree, so an agent file created there (or simply not committed yet) does not exist in the parent's, and scanning the parent would not list it.

**Which directories** are scanned and synced into: the provider's `agent_dir` from `providers.toml` when it has one, otherwise the conventional directory for its command — `claude` → `.claude/agents`, `gemini` and `agy` → `.gemini/agents` (Antigravity reads Gemini's tree), `codex` → `.codex/agents`, `muse` → `.muse/agents`, `opencode` → `.opencode/agents`, `kilo` → `.kilo/agents`. The command is matched on its basename, like the hook bridges and the passive state manifests. A provider with neither a configured directory nor a known command is skipped — set `agent_dir` in the Providers dialog to give it one.
...
crates/
  ...
  desktop/               # piki-desktop — Tauri v2 desktop GUI (piki-core + piki-api-client + piki-agent)
    src/
      ...
      pty_output.rs      # PTY output coalescer (2 ms cold flush / 33 ms sustained / ≤64 KB) + raw IPC channel sink
...
The `vt100` crate is patched to a fork, [`piki-vt100`](https://github.com/juanknebel/piki-vt100) (wired via this repo's root `[patch.crates-io]`), with two behavioral patches plus one addition:
...
2. **`Screen::restore_formatted()`** — serializes scrollback + primary screen + alternate screen + cursor/attrs/input modes to escape codes, so the session daemon can rebuild a terminal on a re-attaching client. Round-trip tests live in [`piki-multiplex`](https://github.com/juanknebel/piki-multiplex)'s `src/session/restore.rs`.
