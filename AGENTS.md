...
**Storage abstraction** (`crates/core/src/storage/`): Trait-based storage layer behind `PIKI_STORAGE` env var. Traits: `WorkspaceStorage`, `ApiHistoryStorage`, `UiPrefsStorage`. Two backends: `JsonStorage` (default, delegates to `workspace::config`) and `SqliteStorage` (`PIKI_STORAGE=sqlite`, single `~/.local/share/piki-multi/piki.db` with WAL mode). `AppStorage` struct holds boxed trait objects; created by `storage::create_storage()` factory. `SqliteStorage` uses `parking_lot::Mutex<Connection>` for thread safety. Schema includes FTS5 virtual table for API history search. Migration from JSON: `piki-multi-ai migrate`.
