...
impl Grid {
    ...
    pub fn scroll_up(&mut self, count: u16) {
        for _ in 0..(count.min(self.size.rows - self.scroll_top)) {
            ...
            // piki-multi patch: real terminals (xterm, kitty, iTerm2,
            // wezterm) push lines scrolled off the top of the screen into
            // scrollback even when a scroll region is active, as long as the
            // region is anchored at row 0. Inline TUI apps (codex, ratatui's
            // `Terminal::insert_before`) rely on this to publish their
            // transcript into the host terminal's native scrollback.
            // Upstream vt100 only saves lines when no region is active.
